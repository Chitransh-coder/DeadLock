 ...
 */
bool DeadLock::isPythonInstalled()
{
#ifdef _WIN32
 ...
 */
bool DeadLock::createVirtualEnv(string venvPath)
{
            ...
            if (packageData.contains("info") && packageData["info"].contains("requires_dist"))
            {
                for (auto &req : packageData["info"]["requires_dist"])
                {
                    if (!req.is_null())
                    {
                        ...
                        if (!depName.empty() && std::find(dependencies.begin(), dependencies.end(), depName) == dependencies.end())
